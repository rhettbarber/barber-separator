	...
	//var channelArray = docRef.channels;
	...
	preferences.rulerUnits = Units.PIXELS;
			 ...
			 refitImage();
              ...
              labelChannels();
                ...
                remindRemoveThreshold();
                    
                    
function remindRemoveThreshold() {
    
        confirm (  ' Remember to run REMOVE_THRESHOLD again using F5 if any any color adjustments are made to the separation' );
...
function addFileName() {
        ...
        try
        {
            ...
            myTextRef.position = new Array( 40, 20 );
            myTextRef.size = 10;
            
			var textColor = new SolidColor(); 
			textColor.cmyk.cyan = 100; 
...
function labelChannels() {
        ...
        // the color used for the text 
			...
			black.cmyk.cyan = 100; 
			black.cmyk.magenta = 100; 
			black.cmyk.yellow = 100; 
        ...
        var currentLayer = doc.activeLayer; 
                ...
                textLayer.textItem.position = new Array( 40, 20 );
            ...
            for(var channelIndex = 0; channelIndex<doc.channels.length; channelIndex++){ 
                ...
                newTextLayer.textItem.contents = doc.channels[channelIndex].name + ' - ' + fileNameNoExtension; 
            ...
            textLayer.remove(); 
                ...
                docRef.flatten();  
                var textLayerCenterMark = doc.artLayers.add(); 
                textLayerCenterMark.kind = LayerKind.TEXT; 
                // font requires the postscript name of the font 
                textLayerCenterMark.font = "ArialMT-Bold"; 
                textLayerCenterMark.textItem.size = new UnitValue(9,'pt'); 
                textLayerCenterMark.textItem.fauxBold = true;
                textLayerCenterMark.textItem.justification = Justification.CENTER; 
                textLayerCenterMark.textItem.capitalization = TextCase.ALLCAPS;
                // set the position for the text. this sets to top right corner of the channel 
                // here it is set so the text baseline ends  40pts from the right edge, 15pts down 
                //textLayer.textItem.position = [new UnitValue(doc.width.as('pt')-8,'pt'),new UnitValue(10,'pt')]; 
                //textLayerCenterMark.textItem.position = new Array( docRef.width / 2, docRef.height - 10);
                textLayerCenterMark.textItem.position = new Array( docRef.width / 2,  10);
                textLayerCenterMark.textItem.contents = '|';// temp label string 


            for(var channelIndex = 0; channelIndex<doc.channels.length; channelIndex++){ 
                var newTextLayerCenterMark = textLayerCenterMark.duplicate(); 
                doc.activeLayer = newTextLayerCenterMark; 
                newTextLayerCenterMark.textItem.contents = '|'; 
                loadActiveLayerTransparencyToSelection(); 
                doc.activeLayer = currentLayer; 
                doc.activeChannels = [doc.channels[channelIndex]]; 
                doc.selection.fill(black); 
                ...
                docRef.flatten();  
                var textLayerCenterMarkBottom = doc.artLayers.add(); 
                textLayerCenterMarkBottom.kind = LayerKind.TEXT; 
                // font requires the postscript name of the font 
                textLayerCenterMarkBottom.font = "ArialMT-Bold"; 
                textLayerCenterMarkBottom.textItem.size = new UnitValue(9,'pt'); 
                textLayerCenterMarkBottom.textItem.fauxBold = true;
                textLayerCenterMarkBottom.textItem.justification = Justification.CENTER; 
                textLayerCenterMarkBottom.textItem.capitalization = TextCase.ALLCAPS;
                // set the position for the text. this sets to top right corner of the channel 
                // here it is set so the text baseline ends  40pts from the right edge, 15pts down 
                //textLayer.textItem.position = [new UnitValue(doc.width.as('pt')-8,'pt'),new UnitValue(10,'pt')]; 
                textLayerCenterMarkBottom.textItem.position = new Array( docRef.width / 2, docRef.height - 10);
                //textLayerCenterMarkBottom.textItem.position = new Array( docRef.width / 2,  10);
                textLayerCenterMarkBottom.textItem.contents = '|';// temp label string 


            for(var channelIndex = 0; channelIndex<doc.channels.length; channelIndex++){ 
                var newtextLayerCenterMarkBottom = textLayerCenterMarkBottom.duplicate(); 
                doc.activeLayer = textLayerCenterMarkBottom; 
                newtextLayerCenterMarkBottom.textItem.contents = '|'; 
                loadActiveLayerTransparencyToSelection(); 
                doc.activeLayer = currentLayer; 
                doc.activeChannels = [doc.channels[channelIndex]]; 
                doc.selection.fill(black); 
                doc.selection.deselect(); 
                selectComponentChannel(); 
                newtextLayerCenterMarkBottom.remove(); 
            }         
        docRef.flatten();
...
function placeRegistration() {
	var docRef = app.activeDocument;
    	preferences.rulerUnits = Units.PIXELS;
            docRef.resizeCanvas( docRef.width, docRef.height + 50);
